			...
			products.map(async (currentProduct) => {
				...
				const updatedPriceHistory: any = [
					...
					})),
					{
						...
						date: currentProduct.priceHistory.length
							? currentProduct.priceHistory[0].date
							: currentDate,
					},
				...
				const newUsers = scrapedProduct.users
					? scrapedProduct.users.filter(
							(scrapedUser: any) =>
								!existingUsers.some(
									(existingUser: any) =>
										existingUser.email === scrapedUser.email
								)
					  )
					: [];
